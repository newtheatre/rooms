...
9. **Erasure is anonymisation.** The GDPR hooks under `server/api/_hooks/auth/` are called by stage-door; they scrub this app's share and are idempotent because stage-door retries them. An erased row carries `anonymised_at` and is never written back over: mirror writes go through `upsertMirroredUser` in `server/utils/mirrorUser.ts` ([ADR-0005](docs/decisions/0005-an-erased-user-is-never-written-back-over.md)).
...
- Errors via `createError`, with no internal detail in responses.
...
Enforced by `bun run check:comments`, which CI runs. There are no exemptions. The same check
enforces the no-em-dash rule across every tracked file, not only comments.
