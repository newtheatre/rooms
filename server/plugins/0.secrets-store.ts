 ...
 *
 * `server/middleware/auth.ts` reads the session and is safe — middleware runs
 * after every plugin `request` hook. It is plugins that need the ordering.
 *
...
export default defineNitroPlugin((nitroApp) => {
  nitroApp.hooks.hook('request', async (event) => {
    ...
    // login page. It cost an evening on 2026-08-14 (stage-door ADR-0016).
    if (!warnedAboutWorkerSecret && process.env.NUXT_SESSION_PASSWORD) {
      ...
      console.error(
        ...
        + 'NUXT_SESSION_PASSWORD --name room-bookings`, then redeploy.'
      )
    }
